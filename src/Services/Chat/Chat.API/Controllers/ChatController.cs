...
{
    [ApiController][Route("api/v1/chat")][Authorize]
    public class ChatController : Controller
    {
        ...
        public async Task<IActionResult> CreateGroup([FromBody] CreateGroupDTO request)
        {
            ...
            var userId = JwtHelper.GetUserIdFromHttpContext(HttpContext); if (userId == 0) return Unauthorized();
        ...
        public async Task<IActionResult> GetMessages(int groupId, [FromQuery] int? cursor = null, [FromQuery] int pageSize = 50)
        {
            ...
            if (result.Message != null && result.Message.StartsWith("NextCursor:"))
                int.TryParse(result.Message.Replace("NextCursor:", ""), out var p);
            return StatusCode(result.StatusCode, new { Data = result.Data, NextCursor = nextCursor, HasMore = nextCursor.HasValue });
        }
